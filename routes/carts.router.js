...
router.post('/:cid/product/:pid', (req, res) => {
    ...
    if (!quantity || quantity <= 0) {
        return res.status(400).send({error: "Cantidad no valida", data: null});
    }

    const cart = cart.find((c) => c.id === parseInt(cid, 10));

    if (!cart) {
        return res.status(404).send({error: "Carrito no encontrado", data: null});
    }

    const productInCart = cart.product.find((p) => p.productId === parseInt(pid, 10));
